...
class OrderAction extends AliziAction {
    ...
    public function index($id,$tpl='detail'){
	...
	$info  = getCache('Item',array('sn'=>$id));
      ...
      if(!empty($template) && preg_match('/^(Alizi\/)/i', $template['template'])){
        ...
        $tpl = ($tpl=='detail' && file_exists(TMPL_PATH.$template['template'].'/detail.html'))?'detail':'detail';
    ...
    public function spu($id,$tpl='spu'){
        if(empty($id) || !ctype_alnum($id))$this->error(lang('error'));
        $info  = getCache('Item',array('sn'=>$id));
        if(empty($info) || $info['is_delete']==1) $this->error(lang('empty_item'));
        $template  = getCache('ItemTemplate',array('id'=>$info['id']),true);
        ...
        if(!empty($template) && preg_match('/^(Alizi\/)/i', $template['template'])){
            ...
            $tpl = ($tpl=='detail' && file_exists(TMPL_PATH.$template['template'].'/spu.html'))?'spu':'spu';
            $tpl = 'Alizi/'.$tplName.$tpl;
            $dir = TMPL_PATH."Alizi/alizi";

            $this->assign('theme',TMPL_PATH.$tplName);
            $this->assign('options',include($dir."/config.php"));
        }
        $info["urls"] = json_encode($this->getUrl($info['sn'],$info['user_id']));
    ...
    public function getUrl($id,$uid){
        $aliziConfig = cache('aliziConfig');
        $aliziConfig['URL_MODEL']= 2;
        if($aliziConfig['URL_MODEL']==2){
            ...
            $url = array(
                'order'=>$this->aliziHost."index.php?m=Order&id={$id}&uid={v}&a=spu",
                'detail'=>$this->aliziHost."index.php?m=Order&id={$id}&uid={$uid}&tpl=detail",
            );
        }
        return $url;
