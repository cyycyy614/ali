...
class ItemAction extends AliziAction {
		...
		foreach ($list as $key => $value) {
			if($aliziConfig['URL_MODEL']==2){
				...
				$url = array(
					...
					'order'=>$this->aliziHost."index.php?m=Order&id={$value['sn']}&uid={$this->uid}&a=spu",
	...
	function edit($id=''){
		...
		if($info){
			...
			$info['colors'] = json_decode($info['colors'],true);
			...
			$uid = $info['account'];
			$userobject = M('User')->where('id='.$uid)->find();
			$info['accountname'] = $userobject["username"];
			if(strstr($info["content"],"{[AliziOrder]}")){

			}else{
				$info["content"] = $info["content"]."<p>{[AliziOrder]}</p>";
			}
		}else{
			$uid = $this->uid;
			$userobject = M('User')->where('id='.$uid)->find();
			$info['accountname'] = $userobject["username"];
			$cont = !!$info["content"] ? $info["content"]: "";
			if(strstr($cont,"{[AliziOrder]}")){

			}else{
				$info["content"] = $cont."<p>{[AliziOrder]}</p>";
			}
		}
		$category = M('Category')->where('type=1')->order('sort_order desc,id asc')->select();
	...
	public function getUrl($id){
		...
		if($aliziConfig['URL_MODEL']==2){
			...
			$url = array(
				'order'=>$this->aliziHost."index.php?m=Order&id={$id}&uid={$this->uid}&a=spu",
