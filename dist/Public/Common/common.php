...
function getUrl($id,$uid){
	$aliziConfig = cache('aliziConfig');

	$prf = "http://{$_SERVER['HTTP_HOST']}".C('ALIZI_ROOT');
	if($aliziConfig['URL_MODEL']==2){
		$url = array(
			'order'=>$prf."single/{$id}-{$uid}.html",
			'detail'=>$prf."detail/{$id}-{$uid}.html",
		);
	}else{
		$url = array(
			'order'=>$prf."index.php?m=Order&id={$id}&uid={$uid}&a=spu",
			'detail'=>$prf."index.php?m=Order&id={$id}&uid={$uid}&tpl=detail",
		);
	}
	return $url;
